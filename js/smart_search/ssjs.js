(function( $ ) {
	$.fn.ssTypeAhead = function(options) {
					...
					success:function (data) {
						...
						for (loop=0; loop<bucketSugs.buckets.length; loop++) {
							...
							for (loopB=0; loopB<bucketSugs.buckets[loop].sugs.length; loopB++) {
								...
								if(options.keywordInterface == 'Standard'){
									$(a1).attr('href', baseURL+'catalogsearch/smart/?q='+bucketSugs.buckets[loop].sugs[loopB].val);
								}
								else{
		...
		$(this).each(function() { 
			...
			var div1 = document.createElement('div');
			$(div1).addClass("ssTypeAheadWrapper");
			$(div1).css({'position':'absolute','width':$(this).width() + 13});
			//$(div1).offset({left:$(this).offset().left,top:($(this).offset().top + $(this).outerHeight(true))});
			$(div1).offset({top:32});
			$(div1).hide();			
			$(this).after(div1);

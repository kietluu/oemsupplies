...
class Escoin_Customer_Model_Customer extends Mage_Customer_Model_Customer
{
    /**#@+
    ...
    public function sendNewAccountEmail($type = 'registered', $backUrl = '', $storeId = '0')
    {
        ...
        $rep_name = Mage::getModel('representative/representative')->load($this->getSalesRepresentative())->getRepresentative();


       
        $attribute = Mage::getModel('eav/config')->getAttribute('customer','province');
         $options = $attribute->getSource()->getAllOptions();
         $prov = $this->getProvince();
                                 foreach($options as $option):
                                        if($option['value'] == $this->getRegion()):
                                            $prov=$option['label'];
                                        endif;
                                    endforeach;
        ...
        $this->_sendEmailTemplate($types[$type], self::XML_PATH_REGISTER_EMAIL_IDENTITY,
            array('customer' => $this,'rep_name'=>$rep_name, 'back_url' => $backUrl), $storeId);
